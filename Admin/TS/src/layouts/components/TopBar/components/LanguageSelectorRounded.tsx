    ...
    <div id="language-selector-rounded" className="topbar-item">
      <Dropdown align="end">
        <DropdownToggle className="topbar-link fw-bold drop-arrow-none" type="button">
          <Image src={selected.flag} alt={selected.name} className="rounded-circle me-2" height={18} id="selected-language-image" />
        ...
        <DropdownMenu className="dropdown-menu-end">
          {languages.map((lang) => (
            <DropdownItem key={lang.code} href="#" onClick={() => handleLanguageChange(lang)} title={lang.title}>
              <Image src={lang.flag} alt={lang.title} className="me-1 rounded-circle" height={18} />
