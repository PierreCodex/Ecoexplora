...
const Chat = () => {
  ...
  return (
    <Card>
      ...
      <SimpleBar className="card-body py-0" style={{ height: 380 }} id="chat-container">
        {chatMessageData.map((item, index) => {
          ...
          return (
            <div key={index} className={`d-flex align-items-start gap-2 my-3 chat-item ${isMe ? 'justify-content-end text-end' : ''}`}>
              {!isMe && <Image src={item.user.image} alt={item.user.name} className="avatar-md rounded-circle" />}
              <div>
